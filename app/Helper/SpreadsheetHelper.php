 ...
 * @package helper
 */
class SpreadsheetHelper extends Base
{
    /**
    ...
    public function render($filename, $content)
    {
        ...
        try {
            $this->assertArchiveIsSafe($tempFile, $extension);
     ...
     *
     * PhpSpreadsheet readers expect a filename, so the content is copied to a
     * temporary file that is removed as soon as the preview is rendered.
     *
     * @access private
    ...
    {
        $filename = tempnam(sys_get_temp_dir(), 'kanboard_spreadsheet_');
     ...
     *
     * Spreadsheet files are ZIP archives. A tiny file can contain a huge
     * amount of uncompressed XML, so the archive is checked before it is
     * handed over to PhpSpreadsheet.
     *
     * @access private
     * @param  string $filename
     * @param  string $extension
     */
    private function assertArchiveIsSafe($filename, $extension)
    {
     ...
     * @return string
     */
    private function escapeMultiline($value)
    {
