...
class FileViewerController extends BaseController
{
    ...
    /**
     * Maximum file size (in bytes) accepted for office document previews.
     * Larger files fall back to a download message instead of being parsed.
     */
    const PREVIEW_MAX_OFFICE_SIZE = 3145728;
    ...
    public function show()
    {
        ...
        $this->response->html($this->template->render('file_viewer/show', array(
            ...
            'params' => $params,
            'type' => $type,
            'content' => ($type === 'spreadsheet' || $type === 'office') ? '' : $this->getFileContent($file, $type),
            'too_large' => (($type === 'text' || $type === 'markdown') && $file['size'] > self::PREVIEW_MAX_SIZE)
                ...
                || ($type === 'office' && $file['size'] > self::PREVIEW_MAX_OFFICE_SIZE),
        )));
    }
    ...
    protected function renderSpreadsheetPreview(array $file)
    {
        ...
        if ($file['size'] > self::PREVIEW_MAX_SPREADSHEET_SIZE) {
            $this->response->html($this->renderPreviewError(t('This file is too large to preview.')));
            return;
        }
        ...
        try {
            ...
            $this->logger->error($e->getMessage());
            $this->response->html($this->renderPreviewError(t('Unable to preview this spreadsheet.')));
        } catch (\Throwable $e) {
    ...
    /**
     * Serve an office document attachment rendered as a sandboxed HTML document
     *
     * The document is generated by DocumentHelper and contains only
     * escaped text. The sandbox directive keeps it script-free even
     * when the URL is opened outside the preview iframe.
     *
     * @access public
     */
    public function office()
    {
        ...
        if ($this->helper->file->getPreviewType($file['name']) !== 'office') {
            throw AccessForbiddenException::getInstance()->withoutLayout();
        }

        $this->renderOfficePreview($file);
    }

    /**
     * Render an office document preview or a user-friendly fallback document
     *
     * @access protected
     * @param  array $file
     */
    protected function renderOfficePreview(array $file)
    {
        $this->response->withHeader('X-Frame-Options', 'SAMEORIGIN');
        $this->response->withHeader('Content-Security-Policy', "sandbox; frame-ancestors 'self'");
        $this->response->withHeader('X-Content-Type-Options', 'nosniff');
        ...
        if ($file['size'] > self::PREVIEW_MAX_OFFICE_SIZE) {
            $this->response->html($this->renderPreviewError(t('This file is too large to preview.')));
            return;
        }
        ...
        try {
            ...
            if (is_string($content) && strlen($content) > self::PREVIEW_MAX_OFFICE_SIZE) {
                $this->response->html($this->renderPreviewError(t('This file is too large to preview.')));
                return;
            }

            $this->response->html($this->helper->document->render($file['name'], $content));
        } catch (ObjectStorageException $e) {
            $this->logger->error($e->getMessage());
            $this->response->html($this->renderPreviewError(t('Unable to preview this document.')));
        } catch (\Throwable $e) {
            $this->logger->error('Unable to render document preview: '.$e->getMessage());
            $this->response->html($this->renderPreviewError(t('Unable to preview this document.')));
        }
    }
     ...
     * @return string
     */
    protected function renderPreviewError($message)
    {
